...
from models.production_run import ProductionRun, ProductionRunUpdate, ProductionRunFinish
...
@production_run_router.get("/",
                           response_model=List[ProductionRun],
                           response_description="List all production runs")
...
@production_run_router.put("/finish/{item_id}", response_model=ProductionRun)
async def finish_production_run(item_id: PydanticObjectId = Path(..., title="The ID of the item to be finished.")) -> ProductionRun:
    body = ProductionRunFinish()
    body.finished = datetime.now(pytz.timezone('Europe/Riga'))
    updated_production_run = await production_run_database.update(item_id, body)
    if not updated_production_run:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Production run with supplied ID does not exist"
